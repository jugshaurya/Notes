...
### 6. Shortest Distance Algorithms
  - _Single Source Shortest Path (SSSP)_
    ...
    - **Dijkstra's Algorithm** <code>O((V+E)logV) = O((V+E)logE)</code>
      ...
      - (can may or may not fails on -ve weighted edges). But it is safe to assume that if question says it has negative edge weights, don't use Dijkstras.
        - Works on TODO image ----------------------------------------(image 1)
        - Doesn't Works on TODO image ----------------------------------------(image 2)
      - surely fails on -ve weighted cycle (stuck in infinte loop or wrong answer depending upon implementation[see Note below]).
...
2. Dijkstra (O((V+E)logE))
  ...
  - else use BellmanFord.
3. Bellman Ford O(VE)
  - will work for negative edge weights. wont stuck in loop for -ve weight cycle.
...
// showing using visited array and dist array together
// on -ve weight graph but no -ve weight cycle
...
int dijkstra(vector<vector<int>>& times, int n, int src) {
  ...
  while(!pqmin.empty()){
      ...
      // these lines improves complxity by many folds
