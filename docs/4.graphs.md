...
- If a **graph is connected** then
  ...
  - Maximum no. of edges = <sup>n</sup>C<sub>2</sub> in undirected,
  - Maximum no. of edges = 2*<sup>n</sup>C<sub>2</sub> in directed,
- **Simple Graph**
...
- **Handshaking Theorem**
  ...
  - In Directed graphs, sum of in-degree = sum of out-degree = |E|.
...
- **Implicit Graph**
  - Graph given in form of 2d Matrix 
    - where there is 4 way connectivity (movement can be top, right, down, left) or
...
- **Handling vertices with value other than integer like strings(the name of city):-**
  - We store the city name and a index in hashmap. (So that finding the index from city name is constant.)
  - along with index and city name in array.(So that finding city name from index is constant.)

- In graphs Problem, We need to mark things `visited`, so that next time if we come back to the same thing we know that we already came there once.

### 3. Cycle Detection can be done using BFS or DFS or DSU.
...
### Bipartite Graphs
  ...
  - Either we can check for odd-length cycle. A graph is Biparttite iff it has no odd-length cycle.
  ...
  - Notation: **K<sub>n,m</sub>** for Bipartitie Graphs whereas Complete graph notation is K<sub>n</sub>
...
- Type of Edges
  ...
  - Back edge: Edge (u, v) such that v is ancestor of edge u but not part of DFS or BFS tree. Edge from node 4 to node 1 is a back edge.
...
- DFS can be used to find Artculation Points and Bridges.
