...
### 2. Special Graphs
  ...
  - **Bipartite Graph**
    ...
    - Notation: <code>K<sub>n,m</sub></code> for Complete Bipartitie Graphs, whereas Complete graph notation is <code>K<sub>n</sub></code>
  - **Regular Graphs**
...
#### Isomorphism
- An Isomorphism b/w graphs G and H is a bijection between the vertex sets of G and H.
- f: V(G)->V(H); such that any two vertices u and v of G are adjacent iff f(u) and f(v) are adjacent in H.
...
- else Have to brute force, try all combinations. Total Choices(#vertices in G *#vertices in G )
...
- Note: We created a Spanning Induced Graph in MST(Krushkal's/Prim)
- #Induced subgraphs possible = 2^|E|

#### Walk, Trial, Circuit, Path, Cycle
- **Walk**
  - Any/Every followed Sequence of vertices is called Walk.
  - [v1, v2, v3, v5, v1, v2] is a walk.
  - [v1, v2, v3, v5, v4] is a walk.
- **Trial**
  - A trial is a walk without repeated edges.
  - ![walkpathtrial](../static/img/graphDocs/walkpathtrial.png) 
- **Path**
...
- **Eulerian path**
  ...
  - Condition that there exist a Euler Circuit
    ...
    - 2. Graph must have `all even degree vertices` or `(n - 2) even and two odd degree vertices`.
