...
- `Containers`
  ...
  - **Container Adapter/ Derived Containers**
    ...
    - Don't support iterators, hence to print the content, use it while the container is an empty thing.
    - (build upon simple containers)
    - (`stack`, `queue`, `deque`, `heap`, `priority queue`)

  - **Associative Containers**
    - Implement a sorted Data structure; data can be searched in O(logn)
    - `set`, `map`, `multimap`, and `multiset`
    - ordered map/set have an order and are implemented as self-balancing BST(Red black Trees).
    - Keys may not need be integral. 
    - Requirement: operator `<` should be defined.

  - **unordered Associative Containers**
    - Implement unordered Data structure; data can be searched in O(1) in avg.
...
- **Important**
  ...
  - `pqmax pq(arr, arr+n)` 
    ...
    - **Inplace Algorithm to convert the array into a heap**
      ...
      - Top-down is unoptimized, bottom-up is optimized.
    ...
    - >> `Bottom-up Approach`
      - This approach is better as the time complexity to build heap is O(n).
      - It builds heap using a bottom-up approach which is O(n).
      - It does it by going backward from last internal node of array floor(n/2) and calls heapify(i).
...
**Tip and Trick:**
  -  Always use map instead of unordered_map as map guarantees to have operations in logn time. 
  - but unordered_map can in worst case goes to O(n). 
...
- You are Awesome and an STL Pro. 🫂