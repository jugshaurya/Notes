...
- pass bool (&fun)(int a, int b) as a parameter. It is a declaration of function + wrapping function name in braces. & is to avoid copy.
...
- Anonymous functions(return type will be inferred as `bool` by the compiler)
...
- The #1 reason why people fail interviews is because of a lack of confidence!
