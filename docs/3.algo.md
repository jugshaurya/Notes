...
- Special Cases
  - if c = 2<sup>n</sup>, Complexity is O(2<sup>n</sup>)
  - if c = n!, Complexity is O(n!)
  - Why? beacuse n<sup>log_a[base_b]</sup> will always be less than n!, 2<sup>n</sup>.
...
4. Wrong Answer
  - check for datatype + unitialized variables.